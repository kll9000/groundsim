...
public sealed class Agent
{
    ...
    private void TickIdle()
    {
        ...
        if (target is null) { _idleCooldown = IdleCooldownTicks; return; }
    ...
    {
        if (Carried is { } material && _sim.Drop(_dropX, Y, material))
        {
            Carried = null;
        }
    ...
    {
        // Approach cell = the Air cell just above the drop column's current
        // surface — recomputed at every (re)plan, so a growing pile simply
        // moves the approach point up.
        int surfY = 0;
        while (surfY < _grid.Height && _grid.IsAir(_dropX, surfY)) surfY++;
        if (surfY == 0) return false; // column solid to the sky
        var approach = (X: _dropX, Y: surfY - 1);
